...
namespace MonoCustomResourceRegistry {
    public static class Settings {
        ...
        static object GetSettings(string title) {
            return ProjectSettings.GetSetting($"{nameof(MonoCustomResourceRegistry)}/{title}");
        }

        static void AddSetting(string title, Variant.Type type, object value, PropertyHint hint = PropertyHint.None, string hintString = "") {
        ...
        static string SettingPath(string title) => $"{nameof(MonoCustomResourceRegistry)}/{title}";
    }
}